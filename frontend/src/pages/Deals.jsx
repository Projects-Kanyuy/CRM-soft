...
export default function Deals() {
  ...
  const fetchData = async () => {
    // Keep loading animation running if we are re-fetching data after an action
  ...
  const handleDragEnd = async (event) => {
    ...
    if (over && active.id !== over.id) {
      ...
      setDeals(currentDeals => currentDeals.map(d => d._id === dealId ? { ...d, stage: newStage } : d));
      try {
  ...
  const handleSave = async (formData) => {
    ...
    try {
      ...
      await fetchData(); // Re-fetch all data to ensure UI is perfectly in sync
    } catch (error) {
    ...
    <div>
      ...
      {loading ? (
        ...
        <DealPipeline 
          ...
          onMoveDeal={handleMove} // <-- Pass the new handler
        />
      )}
      <Modal isOpen={isModalOpen} setIsOpen={closeModal} title={editingDeal ? "Edit Deal" : "Add New Deal"}>
        <DealForm 
          onSave={handleSave} 
          onCancel={closeModal} 
